                        ...
                        <form id="filter-form" ?>
                            <div class="form-row align-items-center">
                                ...
                                <div class="col-md-2 mb-3">
                                    <label for="unor"><strong>Pilih Kesepakatan</strong></label>
                                    <select class="form-control" name="desk2" id="filter-kesepakatan">
                                        <option value="">Semua Kesepakatan</option>
                                        <option value="3">Belum dibahas</option>
                                        <option value="x">ditangguhkan</option>
                                        <option value=1>diakomodasi</option>
                                    </select>
                                </div>
...
<script>
    $(document).ready(function() {
        $('#download-excel').on('click', function() {
            ...
            // Kirim request AJAX
            $.ajax({
                url: '<?= base_url('/desk/excel1') ?>', // Endpoint controller
                type: 'POST',
                data: filterData,
                xhrFields: {
        ...
        $('#filter-kesepakatan, #filter-sumber, #filter-pendanaan').select2();
        ...
        $('#filter-form').on('submit', function() {
            ...
            // Kirim request AJAX
            $.ajax({
                url: '<?= base_url('/desk/laporan1') ?>', // URL untuk memproses filter
                type: 'POST',
                data: filterData,
                success: function(response) {
        ...
        $('#reset-filters').on('click', function() {
            ...
            $('#filter-kesepakatan').val('').trigger('change');
