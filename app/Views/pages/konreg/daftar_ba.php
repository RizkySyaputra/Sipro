...
<div class="card mt-4" id="form-tambah" style="display: none;">
    <div class="card shadow-sm">
        ...
        <div class="card-body">
            <form id="addJabatanForm" method="POST" action="<?= base_url('konreg/add_pejabat') ?>">
                <div class="form-row align-items-center">
                    <!-- Dropdown Pejabat -->
                    ...
                    <input type="hidden" id="hiddenUnorIdForJabatan" name="unor_id" value="">
...
<!-- Modal Peringatan -->
<!-- Modal Peringatan Biru -->
<div class="modal fade" id="warningModal" tabindex="-1" aria-labelledby="warningModalLabel" aria-hidden="true">
    <div class="modal-dialog" style="margin-top: 50px;"> <!-- Dihapus modal-dialog-centered -->
        <div class="modal-content border-primary">
            <div class="modal-body">
                Silakan pilih <strong>Provinsi</strong> dan <strong>UNOR</strong> terlebih dahulu.
            </div>
        </div>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        ...
        $('#filter-form').on('submit', function(event) {
            ...
            let provinsiId = $('#filter-provinsi').val();
            let unorId = $('#filter-unor').val();

            // Validasi input
            if (!provinsiId || !unorId) {
                // Tampilkan modal warning jika belum memilih
            ...
            $('#hiddenUnorIdForJabatan').val(unorId);

            $('#loading-spinner').show();
            $('#button-text').hide();

            $.ajax({
                url: '<?= base_url('/konreg/get_pejabat') ?>',
                type: 'POST',
                data: {
