        ...
        <div class="card">
            <form action="" method="" id="myForm">
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-3 d-flex align-items-center">
                            <input type="text" id="volume" name="volume" class="form-control me-2" value="<?= in_array($jenis_data, ['fkw', 'fkb']) ? $konreg->volume :  $p_memo->volume; ?>" required>
                            <select name="id_satuan" id="list-satuan" class="form-control">
                                <option selected value="<?= in_array($jenis_data, ['fkw', 'fkb']) ? $konreg->id_satuan : $p_memo->id_satuan; ?>"><?= $p_memo->nama_satuan; ?></option>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-9">
                            <input type="text" id="biaya" name="anggaran" class="form-control" value="<?= in_array($jenis_data, ['fkw', 'fkb']) ? $konreg->anggaran :  $p_memo->biaya; ?>" required>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-9">
                            <select name="kd_kgiat" class="form-control" id="select-kd_kegiatan" required>
                                <option value="<?= in_array($jenis_data, ['fkw', 'fkb']) ? $konreg->kd_kgiat : "" ?>" <?= in_array($jenis_data, ['fkw', 'fkb']) ? "" : "disabled" ?> selected> <?= in_array($jenis_data, ['fkw', 'fkb']) ? $konreg->kd_kgiat . " " . $konreg->nmgiat : "-- Pilih Kegiatan --"  ?></option>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-9">
                            <select name="kd_ro" class="form-control" id="select-kd_ro" required>
                                <option value="<?= in_array($jenis_data, ['fkw', 'fkb']) ? $konreg->kd_ro : "" ?>" <?= in_array($jenis_data, ['fkw', 'fkb']) ? "" : "disabled" ?> selected> <?= in_array($jenis_data, ['fkw', 'fkb']) ? $konreg->kd_ro . " " . $konreg->nmro : "-- Pilih Ro --"  ?></option>
                                <?php foreach ($ro as $item) : ?>
                                    <option value="<?= $item['kdro'] ?>"><?= $item['kdro'] . " " . $item['nmro'] ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-9">
                            <select name="id_tematik" class="form-control" id="select-tematik">
                                <option value="<?= in_array($jenis_data, ['fkw', 'fkb']) ? $konreg->id_tematik : "" ?>" <?= in_array($jenis_data, ['fkw', 'fkb']) ? "" : "disabled" ?> selected> <?= in_array($jenis_data, ['fkw', 'fkb']) ? $konreg->tematik : "-- Pilih Tematik --"  ?></option>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-9">
                            <textarea style="height: auto;" name="catatan_pradesk" class="form-control"><?= in_array($jenis_data, ['fkw', 'fkb']) ? $konreg->catatan_pradesk : "" ?></textarea>
                        </div>
                    </div>
                ...
                <div class="col-sm-12 text-center">
                    <button type="submit" class="btn filter" id="submitBtn"><?= (in_array($jenis_data, ['fkw', 'fkb'])) ? "Simpan Perubahan" : "Proses Data"; ?> </button>
                </div>
        </div>
        </form>
    </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#select-tematik, #select-renc_induk,#select-kd_kegiatan, #select-kd_kro, #select-kd_ro, #select-fs, #select-dokling, #select-lahan, #select-ded, #select-pasca_kons, #select-terima_bantuan').select2();
        ...
        $('#submitBtn').click(function(e) {
            ...
            // Kirim pakai AJAX
            $.ajax({
                url: "<?= base_url('/prosesRakorbangwil') ?>",
                method: "POST",
                data: formData,
                processData: false,
                contentType: false,
                success: function(res) {
                    ...
                    swal({
                        ...
                        window.location.href = "<?= base_url('/daftarRakorbangwil') ?>";
                    });
                },
                error: function(err) {
                    swal({
