    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header card-header-primary card-header-icon">
                ...
                <h4 class="card-title font-weight-bold"><strong>Program Konreg FKW</strong></h4>
                    ...
                    <div class="card-body">
                        <form id="filter-form" ?>
                            <div class="form-row align-items-center">
                                ...
                                <div class="col-md-2 mb-3">
                                    <label for="Kesepakatan"><strong>Pilih Kesepakatan</strong></label>
                                    <select class="form-control" name="kesepakatan" id="filter-Kesepakatan">
                                        <option value="">Semua Kesepakatan</option>
                                        <option value="0">Non FKS/Belum Terbahas</option>
                                        <option value="1">FKS</option>
                                ...
                                <!-- <div class="col-md-2 mb-3">
                                    <label for="pradesk"><strong>Pradesk Konreg</strong></label>
                                    <select class="form-control" name="pradesk" id="filter-pradesk">
                                        <option value="">Semua Sumber</option>
...
<script>
    $(document).ready(function() {
        $('#download-excel').on('click', function() {
            ...
            // Kirim request AJAX
            $.ajax({
                url: '<?= base_url('/fkw/exportToExcel') ?>', // Endpoint controller
                type: 'POST',
                data: filterData,
                xhrFields: {
                ...
                success: function(response) {
                    ...
                    link.download = 'Program_FKW' + timestamp + '.xlsx'; // Nama file unduhan
                    link.click();
                    Swal.fire({
                        icon: 'success',
                        title: 'Success',
        ...
        $('#filter-unor, #filter-provinsi, #filter-Kesepakatan, #filter-pendanaan, #filter-pradesk, #filter-sumber').select2();
        ...
        $('#filter-form').on('submit', function() {
            ...
            // Kirim request AJAX
            $.ajax({
                url: '<?= base_url(($desk) ? '/program/get_program_fkw?desk=konreg' : '/program/get_program_fkw') ?>', // URL untuk memproses filter
                type: 'POST',
                data: filterData,
                success: function(response) {
