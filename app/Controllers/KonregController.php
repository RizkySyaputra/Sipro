...
use App\Models\UserProvinsiModel\UserProvinsiModel as ModelsUserProvinsiModel;
...
class KonregController extends BaseController
{
    ...
    public function export_to_excel_ditangguhkan()
    {
        ...
        if ($sumber == 'All') {
            $awal_data = 'Seluruh Data';
            $provinsi = $this->usulanProvinsiModel->getUsulan($provinsi_id, $unor_id, null, 3);
            $rakortek = $this->apiRakortek->getProgram($provinsi_id, $unor_id, null, 3);
            $fkw = $this->FkwModel->getProgram($provinsi_id, $unor_id, 2);
            $fkb = $this->FkbModel->getProgram($provinsi_id, $unor_id, 2);
        } elseif ($sumber == 'Rakorbangwil') {
            $awal_data = 'Rakorbangwil';
            $fkw = $this->FkwModel->getProgram($provinsi_id, $unor_id, 2, $sumber);
            $fkb = $this->FkbModel->getProgram($provinsi_id, $unor_id, 2, $sumber);
        } elseif ($sumber == 'Rakortek') {
            $awal_data = 'Rakortek';
            $fkw = $this->FkwModel->getProgram($provinsi_id, $unor_id, 2, $sumber);
            $fkb = $this->FkbModel->getProgram($provinsi_id, $unor_id, 2, $sumber);
            $rakortek = $this->apiRakortek->getProgram($provinsi_id, $unor_id, null, 3);
            ...
            // d($fkw);
            // dd($fkb);
        } elseif ($sumber == 'Usulan Provinsi') {
            $awal_data = 'Usulan Provinsi';
            $fkw = $this->FkwModel->getProgram($provinsi_id, $unor_id, 2, $sumber);
            $fkb = $this->FkbModel->getProgram($provinsi_id, $unor_id, 2, $sumber);
            $provinsi = $this->usulanProvinsiModel->getUsulan($provinsi_id, $unor_id, null, 3);
        } elseif ($sumber == 'Unor') {
        ...
        foreach ($fkw as &$item) {
            $item->sumber_data = 'FKW';
        }
        foreach ($fkb as &$item) {
            $item->sumber_data = 'FKB';
        }
        foreach ($provinsi as &$item) {
            $item->sumber_data = 'FUP';
        }
        foreach ($rakortek as &$item) {
            $item->sumber_data = 'Rakortek';
        }
        $programs = array_merge($provinsi, $rakortek, $fkw, $fkb);
        ...
        $sheet->setCellValue('G1', 'Kesepakatan');
        $sheet->setCellValue('H1', 'Jenis Program');
        ...
        foreach ($programs as $index => $data) {
            $sheet->setCellValue('A' . $row, $index + 1); // No
            $sheet->setCellValue('B' . $row, $data->provinsi ?? '-');
            $sheet->setCellValue('C' . $row, $data->unor ?? '-');
            ...
            $sheet->setCellValue('D' . $row, $namaPekerjaan);
            ...
            $sheet->setCellValue('E' . $row, $volume . ' ' . ($data->nama_satuan ?? ''));
            ...
            $formattedAnggaran = $fmt->formatCurrency($anggaran, 'IDR');
            $sheet->setCellValue('F' . $row, $formattedAnggaran);

            // Status (hardcoded: Ditangguhkan)
            $sheet->setCellValue('G' . $row, 'Ditangguhkan');

            // Sumber data
            $sheet->setCellValue('H' . $row, strtoupper($data->sumber_data ?? '-'));
            $sheet->setCellValue('I' . $row, strtoupper($awal_data ?? '-'));
        ...
        // Simpan file sebagai output langsung
        $writer = new Xlsx($spreadsheet);
        $filename = 'Program_Ditangguhkan' . date('Y-m-d_H-i-s') . '.xlsx';
    ...
    public function generateBA($id_provinsi, $id_unor, $tanggal) //ganti dengan id provinsi
    {
        ...
        $lampiranFKW = $this->FkwModel->getProgramFKWLampiran($id_provinsi, $id_unor, 1);
        $lampiranFKB = $this->FkbModel->getProgramFKBLampiran($id_provinsi, $id_unor, 1);
        // d($lampiranFKW);
        // dd($lampiranFKB);
        ...
        $html = view('BA_Konreg', [
            ...
            'fkw' => $programsFKW,
            'fkb' => $programsFKB,
            'fkwLampiran' => $lampiranFKW,
            'fkbLampiran' => $lampiranFKB,
            'tanggal' => $tanggal,
    ...
    public function get_pejabat_ba()
    {
        ...
        $id_unor = $this->request->getPost('unor_id');
        $pejabat = $this->baModel->getPejabatById($id_provinsi, $id_unor);
        $data = [
            'pejabat' => $pejabat
        ];
        return view('/pages/konreg/tabel/tabel_pejabat_ttd', $data);
    }
    ...
    public function add_pejabat_ba()
    {
        if ($this->request->getMethod() === 'POST') {
            ...
            $UnoriId = $this->request->getPost('unor_id');
    ...
    public function get_laporan1()
    {
        $jenis_data = $this->request->getPost('jenis_data');
        $sumber = $this->request->getPost('sumber');
        $rekap_fkw = $this->FkwModel->getProgramLaporan2($sumber, 1);
        $rekap_fkb = $this->FkbModel->getProgramLaporan2($sumber, 1);
        ...
        if ($jenis_data == 'fkw') {
            ...
            // Masukkan data FKW ke array gabungan
            foreach ($rekap_fkw as $item) {
                $id = $item->id_provinsi;
                $gabungan[$id] = (array) $item; // Cast ke array agar bisa diolah
            }

            // Tambahkan data FKB ke array gabungan
            foreach ($rekap_fkb as $item) {
                $id = $item->id_provinsi;

                if (!isset($gabungan[$id])) {
                    $gabungan[$id] = (array) $item;
                } else {
                    // Gabungkan nilai-nilainya
                    foreach ($item as $key => $value) {
                        if ($key === 'id_provinsi' || $key === 'provinsi') {
                            continue; // Lewati kolom identitas
                        }

                        $gabungan[$id][$key] += $value;
                    }
                }
            }

            // Jika perlu ubah kembali ke objek (opsional)
        ...
        return view('/pages/konreg/tabel/tabel_laporan_program', $data);
    }

    public function laporan1()
    {
        $this->template->write('title', 'Laporan Program Konreg');
        $this->template->load('/templates/main', '/pages/konreg/program/rekap_program_fkb_fwk');
    }
    public function rekap_fks()
    {
        $dataProvinsi = $this->provinsiModel->getProvinsi();
        $dataUnor = $this->unorModel->getUnor();
        $data = [
        ...
        $this->template->write('title', 'Program Konreg FKW');
        $this->template->load('/templates/main', '/pages/konreg/rekap/rekap_program_fks', $data);
    }

    public function get_rekap_fks()
    {
        $unor_id = $this->request->getPost('unor');
        $provinsi_id = $this->request->getPost('provinsi');
        $sumber = $this->request->getPost('sumber');
        $fkw = $this->FkwModel->getProgram($provinsi_id, $unor_id, 1, $sumber);
        $fkb = $this->FkbModel->getProgram($provinsi_id, $unor_id, 1, $sumber);

        // Tambahkan sumber data ke setiap item
        foreach ($fkw as &$item) {
            $item->sumber_data = 'FKW';
        }
        foreach ($fkb as &$item) {
        ...
        $programs = array_merge($fkw, $fkb);
        $data = [
            'program' => $programs
        ];
        return view('/pages/konreg/rekap/tabel_program_fks', $data);
    }

    public function rekap_ditangguhkan()
    {
        $dataProvinsi = $this->provinsiModel->getProvinsi();
        $dataUnor = $this->unorModel->getUnor();
        $data = [
        ...
        $this->template->write('title', 'Program Konreg FKW');
        $this->template->load('/templates/main', '/pages/konreg/rekap/rekap_program_ditangguhkan', $data);
    }

    public function get_rekap_ditangguhkan()
    {
        $unor_id = $this->request->getPost('unor');
        $provinsi_id = $this->request->getPost('provinsi');
        $sumber = $this->request->getPost('sumber');
    ...
    public function pra_rakorbangwil()
    {
        $dataKesepakatan = $this->kesepakatanModel->where('kegiatan', 'Pra Konreg')->findAll();
        $id_user = user()->id;
        ...
        $this->template->load('/templates/main', '/pages/konreg/praKonreg/rakorbangwil', [
            ...
            'can_edit' => has_permission_menu($id_role, '/konreg/pra_konreg/rakorbangwil', 'can_edit'),
            'can_delete' => has_permission_menu($id_role, '/konreg/pra_konreg/rakorbangwil', 'can_delete')
        ]);
    }
    public function get_pra_konreg_rakorbangwil()
    {
        ...
        return view('/pages/konreg/praKonreg/tabel/tabel_desk_daftar_program_tahunan', $data);
    }

    public function view_rakorbangwil($id)
    {
        ...
        return view('/pages/konreg/praKonreg/ModalView', [
    ...
    public function edit_rakorbangwil($id)
    {
    ...
    public function update_rakorbangwil($id)
    {
        ...
        $dataToUpdate = [
            'catatan_pra_konreg' => json_encode($Catatan, JSON_UNESCAPED_UNICODE),
            'tipe_pekerjaan'            => $tipe_pekerjaan,
            'pra_konreg'         => $kesepakatan,
